...
//echo "<script>alert('".$to_date."');</script>";		
	...
	$result = 0;
        ...
        $data = array(
                ...
                'hra_bonus1'  => $this->input->post('hra_bonus1'), 
                'hra_bonus2'  => $this->input->post('hra_bonus2'), 
            );
	...
	$this->db->or_where(' id !="'.$id.'" AND  to_date   BETWEEN "'.$from_date. '" and "'.$to_date.'" ');
        ...
        $this->db->set('hra_bonus1', $hra_bonus1);
        $this->db->set('hra_bonus2', $hra_bonus2);
        $this->db->where('id', $id);
...
$challan_date = $this->db->query('select count(wage_month) as countdate from challan_date_entry where wage_month="'.$month_year.'" and return_date>="'.$today_date.'" ');
	...
	$row .= '####'.$ac1eemf;	   
	$row .= '####'.$ac10;	   
	$row .= '####'.round((float)$wages);	   
	$row .= '####'.round((float)$bonus);	   
	$row .= '####'.round((float)$total);	   
	$row .= '####'.round((float)$pf);	   
	$row .= '####'.round((float)$net_wages);	   
	$row .= '####'.round($unit_days1);	   
	$row .= '####'.round($unit_days2);	   
	$row .= '####'.round($no_of_days);	   
	$row .= '####'.round((float)$leave_with_pay);	   
	$row .= '####'.$member_id;
	$row .= '####'.$ac1eemale;
	$row .= '####'.$ncp_days1;
	$row .= '####'.$status;
