...
//		echo $query;
			...
			if ($query->num_rows() > 0) {
				...
				if (($previus_member_id != "") && ($previus_member_id != "NOT AVAILABLE")) {
					$this->db->set('member_id', $previus_member_id);
				}
				if (($dob1 != "") && ($dob1 != "NOT AVAILABLE")) {
					$this->db->set('dob', $dob1);
				}
				if (($doj1 != "") && ($doj1 != "NOT AVAILABLE")) {
					$this->db->set('doj', $doj1);
				}
				if (($gender != "") && ($gender != "NOT AVAILABLE")) {
					$this->db->set('gender', $gender);
				}
				if (($father_husband != "") && ($father_husband != "NOT AVAILABLE")) {
					$this->db->set('father_husband', $father_husband);
				}
				if (($relation != "") && ($relation != "NOT AVAILABLE")) {
				...
				if (($emp_name != "") && ($emp_name != "NOT AVAILABLE")) {
					$this->db->set('name_as_aadhaar', $emp_name);
				}
				if (($pmid != "") && ($pmid != "NOT AVAILABLE")) {
					$this->db->set('member_id_org', $pmid);
				}
				if (($contractor_id != "") && ($contractor_id != "NOT AVAILABLE")) {
					$this->db->set('contractor', $contractor_id);
				}
				if (($emptype != "") && ($emptype != "NOT AVAILABLE")) {
					$this->db->set('employee_type', $emptype);
				}
				if (($nationality != "") && ($nationality != "NOT AVAILABLE")) {
					$this->db->set('nationality', $nationality);
				}
				if (($pmrpy != "") && ($pmrpy != "NOT AVAILABLE")) {
					$this->db->set('pmrpy', $pmrpy);
				}
				if (($pmid != "") && ($pmid != "NOT AVAILABLE")) {
					$this->db->set('member_id_org', 'WBDGP0034083000' . $pmid);
				}
				if (($ip_number != "") && ($ip_number != "NOT AVAILABLE")) {
